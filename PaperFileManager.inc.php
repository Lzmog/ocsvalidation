...
class PaperFileManager extends FileManager {
	...
	function handleUpload($fileName, $type, $fileId = null, $overwrite = false) {
        ...
        $mime_type = array('application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/msword');
        $mime_name = array('docx','doc');

        $path = $_FILES[$fileName]['name'];
        $file_name = pathinfo($path, PATHINFO_EXTENSION);

        $file_type = $_FILES[$fileName]['type'];

        if (in_array($file_name,$mime_name) && in_array($file_type,$mime_type))
        {
            if (!$fileId) {
